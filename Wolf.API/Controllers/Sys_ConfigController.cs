...
namespace Wolf.API.Controllers
{
    public class Sys_ConfigController : ApiControllerBase<Sys_Config>
    {
        ...
        public Sys_ConfigController(IServiceWrapper repository, ILogger<Sys_CategoryController> logger) :base(repository, logger)
        {
            _logger = logger;
            _service = repository;
        }
        ...
        public virtual async Task<IActionResult> GetListPagedByType(int page = 1, int pageSize = 10, int totalLimitItems = 500, int type = 0)
        {
            try
            {
                ...
                var items = await _service.Sys_Config.GetPagedAsync(page, pageSize, totalLimitItems, search);
